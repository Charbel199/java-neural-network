...
import mnist.Mnist;
import static mnist.Mnist.createTrainSet;
...
public class Network{
    ...
    public static void testTrainset(Network net, TrainSet set,int printsteps){
        ...
        for(int i=0;i<set.size();i++){
            ...
            if (highest==actualHighest){correct++;
                
            }
            if (i%printsteps==0){
                System.out.println(i+": "+(double)correct/(double)(i+1));
            }
        }System.out.println("Testing done,RESULTS: "+correct+" / "+set.size()+" -> "+(double)correct/(double)(set.size()));
    ...
    public static void trainData(Network net,TrainSet set, int epochs,int loops,int bach_sizes){
        for(int e=0;e<epochs;e++){
            net.train(set,loops,bach_sizes);
    ...
    TrainSet set;
     //   set = createTrainSet(0,9999);
//trainData(net,set,20,50,100);

   TrainSet testset;
        set = createTrainSet(9999,19999);
